...
describe Event do
  ...
  let (:event_catering) do
    ...
    e1.add_replace_menu_template(populated_menu_template_catering, 40, false)
    e1
  end
  ...
  context "when in creation process" do
    ...
    it "should create the correct number of vendor Menu-Fee line items 2"  do
      ...
      it "should have the correct cogs value in all inventory items " + count.to_s do
        event_catering.line_items.select{|li| li.payable_party == event_catering.event_vendors.first.vendor && li.line_item_sub_type == "Menu-Item"}[count].unit_price_expense.should == populated_menu_template_catering.inventory_items[count].cogs
      end
    }
