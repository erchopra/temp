...
describe IssuesController do
  ...
  describe "Post 'create'" do
    it "should create issue for" do
      ...
      response.should redirect_to("/vendors/#{subject.id}?selected=issues")
    end
  ...
  describe "Put 'update'" do
    it "should update issue" do
      ...
      expect(response.status).to eq(302)
      response.should redirect_to("/#{issue.subject.class.name.downcase}s/#{issue.subject.id}?selected=issues")
    end
