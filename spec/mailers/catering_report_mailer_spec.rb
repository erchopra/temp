...
describe CateringReportMailer do
  let(:user) { mock_model(User, username: 'Dave', email: 'dave.bremenr2@gcatering_mail.com') }
  let(:event) { create :catering_event }
  
  describe 'catering_schedule_preview' do
    let(:catering_mail) { CateringReportMailer.catering_schedule_preview(user, [event], "Dave", "david.bremner@fooda.com", "Test") }
    ...
    it 'renders the sender email' do
      catering_mail.from.should == ['snappea-automation@fooda.com']
    end
  ...
  describe 'end_of_day_sales_report' do
    let(:sales_mail) { CateringReportMailer.end_of_day_sales_report(user, [event], "Bob", "david.bremner@fooda.com", "Test") }
    ...
    #ensure that the sender is correct
    it 'renders the sender email' do
      sales_mail.from.should == ['snappea-automation@fooda.com']
    end
