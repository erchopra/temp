...
#  white_label_fooda               :boolean          default(FALSE), not null
...
class EventVendor < ActiveRecord::Base
  ...
  public
    ...
    def invoiced_line_items?
      event.line_items.select{|li| !li.document_id.nil? && (li.payable_party == self.vendor || li.billable_party == self.vendor)}.count > 0
    end
