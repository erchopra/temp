module Event::LineItems
  ...
  def line_items_per_person_charge_by_payable_party party, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.payable_party == party && li.line_item_sub_type == "Menu-Fee" && !li.after_subtotal)}, invoiced, document_id)
  end

  def line_items_no_price_menu_items_by_payable_party party, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.payable_party == party && li.line_item_sub_type == "Menu-Item" && !li.include_price_in_expense && !li.after_subtotal)}, invoiced, document_id)
  ...
  def line_items_non_menu_items_by_payable_party party, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.payable_party == party && li.line_item_sub_type != "Menu-Item" && li.line_item_sub_type != "Menu-Fee" && !li.after_subtotal)}, invoiced, document_id)
  end

  def line_items_after_subtotal_by_payable_party party, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.payable_party == party && li.after_subtotal)}, invoiced, document_id)
  end

  # By Billable Party
  # ---------------------------------------------------------------------------
  def line_items_vendor_per_person_charge_by_billable_party party, vendor, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.billable_party == party && li.per_person_price_for_vendor(vendor) && !li.after_subtotal)}, invoiced, document_id)
  end

  def line_items_vendor_no_price_menu_items_by_billable_party party, vendor, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.billable_party == party && li.menu_item_for_vendor(vendor) && !li.include_price_in_revenue && !li.after_subtotal)}, invoiced, document_id)
    .sort_by{|e| [MealType.available_values.index(e.inventory_item.meal_type)]}
  end
  ...
  def line_items_no_associated_vendor_by_billable_party party, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.billable_party == party && !li.associated_with_vendor && !li.after_subtotal)}, invoiced, document_id)
  end
  ...
  def line_items_after_subtotal_percentage_by_billable_party party, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.billable_party == party && li.after_subtotal && li.percentage_of_subtotal)}, invoiced, document_id)
  end
  ...
  def line_items_after_subtotal_by_billable_party party, invoiced=nil, document_id=nil
    filter_invoiced(line_items.select{|li| (li.billable_party == party && li.after_subtotal)}, invoiced, document_id)
  end
